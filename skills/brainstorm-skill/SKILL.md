...
description: 하네스 시작 시 참가 연구대회와 분야를 묻고 연구 동향을 리서치해 가장 적합한 연구 주제와 보고서 제목을 브레인스토밍해 input/ideas/에 자동 작성한다. 사람이 ideas 파일을 직접 쓰지 않는다.
...
## 무엇을 하나
1. **인터뷰**: 참가 연구대회명, 분야/교과(필수), 학교급/학년, 상황, 관심 트렌드, 활용 도구, 목표 역량, 제약을 묻는다.
...
5. **자동 작성**: `input/ideas/`에 인터뷰·트렌드·주제·제목 파일과 `brainstorm.json`을 쓰고, `input/rules/competition-profile.json`에 대회 프로필을 저장한다.
6. **선택 배경연구**: `--research-background`를 붙이면 주제 선정 직후 `rch research-background`를 이어 실행한다.

## 실행
```bash
rch brainstorm my-competition                      # 대화형
rch brainstorm my-competition --competition-name "과학전람회"
