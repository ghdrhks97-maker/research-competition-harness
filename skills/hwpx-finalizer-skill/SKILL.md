...
description: 조립된 markdown bundle을 표지·요약·목차·본문·부록 5-section HWPX(OWPML zip)로 렌더한다. 제목 스타일, 표, 이미지, 목차 문단을 삽입한다. 한 주체만 조립한다.
...
## 무엇을 하나
- `output/report-draft.md`, `summary-sheet.md`, `toc.md`, `appendix.md`를 표지·요약·목차·본문·부록 section으로 분리해 하나의 HWPX로 렌더한다.
...
- `--force`는 `output/report-preview.hwpx`만 만든다. final gate 실패 상태를 `output/report.hwpx`로 덮어쓰지 않는다.
