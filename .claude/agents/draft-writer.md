...
## 분량 규격 (필수 — 심사규정이 기준)
**집필 전에 `input/rules/`(공문·심사표·보고서 양식)를 먼저 읽고 이 세 가지를 추출한다:**
1. **요구 목차/항목 구조** — 양식이 정한 장·절 구성을 그대로 따른다(임의로 I~V로 바꾸지 않는다).
2. **분량 규정** — 본문 상한과 제외 항목(예: "표지·목차·요약서 제외 25쪽 이내"). 상한을 넘기지 않되 **상한에 최대한 가깝게 꽉 채운다**(상한의 90% 이상 목표). 골격만 쓰고 끝내면 실패.
3. **심사표 배점** — 배점이 큰 항목에 분량을 비례 배정한다.

추출한 규격은 **`lanes/draft-writer/agent/plan.md`에만** "분량 계획표: 항목→목표 쪽수→근거(배점)"로 기록한다. **본문(lane-output.md)에는 계획표·집필 메모를 절대 넣지 않는다** — 본문에 남으면 `rch build-hwpx`가 빌드를 거부한다. `rch render-check --min-pages <상한의 90%>`로 하한을, `--page-limit <상한>`으로 상한을 추정 검사한다.

**"(생략)", "…표 삽입 예정" 같은 문구로 표·내용을 건너뛰는 것은 금지다.** 표가 계획됐으면 그 자리에서 실제 표를 완성한다. 이런 문구가 남으면 빌드 게이트에 걸려 HWPX가 나오지 않는다.

규정에 목차·분량이 없을 때만 아래 기본 예산(본문 25쪽 기준)을 쓴다:

| 장 | 분량 | 채우는 방법 |
| --- | --- | --- |
| I. 필요성·목적 | 2~3쪽 | 실태 통계 인용, 문제 장면 서사, 연구 질문 박스, 운영 개요표 |
| II. 이론적 배경·실태 | 3~4쪽 | `input/research/background-research.md`의 개념 정의·이론·실태·선행연구 표·차별성 (**전부 사용**) |
| III. 수업 설계·실천과제 | 5~6쪽 | 약어 모형 글자별 풀이표, 실천과제별 설계표, 차시 운영표, 평가·루브릭 연계표 |
| IV. 실천 과정·결과 | 9~11쪽 | **실천과제마다 동일 구조 반복**: 목표→활동 전개(차시 표)→학생 활동 장면 서술→산출물→변화 근거(설문 표+해석+정성 근거) |
| V. 결론·제언·확산 | 2~3쪽 | 성과 요약표, 한계, 일반화 조건, 확산 계획 |

미달 위험이 보이면 실천·결과 장의 서술을 활동 단위로 더 세분해 채운다(빈말 반복 금지 — 구체 장면·산출물·해석으로).

## 산출물 (lanes/draft-writer/agent/ 에 4개 모두 작성)
1. `lane-output.md` — I~V장 본문. 각 장:
   - **`input/rules/`의 보고서 양식·심사표가 있으면 그 목차·항목 구조를 그대로 따른다**(I~V는 기본값일 뿐). 심사표 배점이 큰 항목에 분량을 더 배정.
   - **표가 말하고 본문이 해석하는 구조**. 긴 설명은 3~5열 표/단계 카드로.
   - 연구 질문·수업 모형 정의·핵심 결과 같은 강조 내용은 **`:::box 제목` ... `:::` 지시문**으로 감싼다(HWPX에서 색 박스로 렌더됨). `lanes/icon-visual/agent/lane-output.md`의 **아이콘 사용 규칙**(글리프 배정)을 그대로 따른다 — 없으면 ▶ ◆ ■ 기본 리듬.
