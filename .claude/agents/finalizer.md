...
4. 가능하면 **`rch visual-check`로 PDF/page evidence까지 남긴다.**
5. **render-check 통과 = 구조만. 한컴 실제 표시가 아니다.** 반드시 사용자에게 한컴에서 열어 확인하라고 요구한다.
...
어느 경로든 빌드 후 `rch render-check` → 가능하면 `rch visual-check` → 한컴 육안 확인 안내는 동일하다. kordoc은 오픈소스(https://github.com/chrisryugj/kordoc, Node 18+)이며 없으면 설치를 사용자에게 제안만 하고 3번으로 진행한다(멈추지 않는다).
...
## 빌드·검증 루프 (강제)
```
rch build-hwpx <ws>       # 사전 품질 게이트 내장: 금지어·"(생략"·집필 메모·최소 분량 미달이면 빌드 거부
rch render-check <ws> --page-limit <규정 상한> --min-pages <상한의 90%>
rch visual-check <ws>     # RHWP/Hancom 호환 렌더러가 있으면 PDF/page metrics 생성
rch diagnose <ws>         # 구버전 렌더러·골격 신호 최종 확인
# 예: 표지·목차·요약서 제외 25쪽 규정 → --page-limit 25 --min-pages 22
# output/render-check.json 을 읽는다
```
분량 규정은 `input/rules/`에서 읽는다(없으면 25/22). **빌드 거부·미달 경고가 나오면 해당 마커를 지우는 게 아니라 draft-writer에 실제 내용 보강을 재위임**한다(압축만이 아니라 채우기도 게이트다). `--force`는 중간 확인용일 뿐 최종본에 쓰지 않는다.
render-check JSON에서 **모두** 확인:
- `ok == true`, `errors == []`
- `section_count >= 4`, `section_files`에 표지·요약·목차·본문·부록 section이 존재
- 모든 section에 **페이지 정의(hp:pagePr) 존재** — 없으면 한컴에서 빈 문서로 보인다(→ 최신 코드로 `rch build-hwpx` 재실행)
- `paragraph_count > 0`, `table_count`가 기대와 일치, `heading_count > 0`
- `estimated_pages <= page_limit`(초과 시 table-layout에 압축 재위임)
- `estimated_pages >= min_pages` 권장. 미달이면 draft-writer에 실제 본문 보강 재위임
...
## 디자인 인계 (구조 통과 후)
render-check `ok:true`가 되면 **`hwpx-designer`에게 인계**한다 — 표지·장 도비라·색 박스·아이콘·카드형 요약을 `rch hwpx-unpack`→편집→`rch hwpx-pack`→`rch visual-check` 반복으로 입히는 역할. 당신은 구조·정합성 책임, 디자인 반복은 hwpx-designer 책임.
...
- 사용자에게 **반드시** 보고: render-check 결과 요약 + visual-check 결과(있으면 PDF/page metrics) + "한컴/HOP에서 `output/report.hwpx`(또는 report.docx)를 열어 **페이지 수·표 흐름·이미지 겹침·목차 번호**를 직접 확인하세요."
